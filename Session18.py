...
def executeSQL(sql):
    con = lib.connect(user="root", password="", database="customer", host="127.0.0.1", port="3306")
...
while repeat == "yes":
    ...
    if choice == 1:
        ...
        if save == "yes":
            sql = "update Customer set name = '{}', phone='{}', email= '{}' where id = {}".format(customer.name,
                                                                                                  customer.phone,
                                                                                                  customer.email,
                                                                                                  customer.id)
            executeSQL(sql)
        ...
        con = lib.connect(user="root", password="", database= "company", host="127.0.0.1", port="3306")
        cursor = con.cursor()
        cursor.execute(sql)
